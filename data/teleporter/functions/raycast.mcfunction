...
execute positioned ^ ^ ^0.1 rotated ~ ~ unless block ~ ~ ~ #teleporter:raycast_pass run function teleporter:block_collide 

scoreboard players add .distance rc 1

# Make raycast visible
particle minecraft:portal ~ ~ ~ 0 0 0 0 0 force
...
execute if score .distance rc matches ..2000 positioned ^ ^ ^0.1 rotated ~ ~ if block ~ ~ ~ #teleporter:raycast_pass run function teleporter:raycast
